...
public partial class Gunfig
{
    ...
    /// <param name="callback">An optional Action to call when changes to the scrollbox are applied. The callback's first argument will be the scrollbox's <paramref name="key"/>. The callback's second argument will be the scrollbox's applied value.</param>
    ...
    public void AddScrollBox(
        string key,
        List<string> options,
        string label = null,
        Action<string, string> callback = null,
        List<string> info = null,
        Gunfig.Update updateType = Gunfig.Update.OnConfirm,
        string defaultValue = null,
        bool callbackImmediately = false,
        Action<string, string> pendingValueChanged = null,
        List<string> selectableValues = null)
    {
        if (options == null || options.Count == 0)
            throw new ArgumentException(
                "Scroll box options must contain at least one value.",
                nameof(options));
        ...
        if (selectableValues != null)
        {
            if (selectableValues.Count == 0)
                throw new ArgumentException(
                    "Scroll box selectable values must contain at least one value.",
            ...
            foreach (string value in selectableValues)
            {
                if (!options.Contains(value))
                    throw new ArgumentException(
                        $"Selectable value '{value}' is not present in the scroll box options.",
                        nameof(selectableValues));
            }
        }
